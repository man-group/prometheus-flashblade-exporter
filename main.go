...
var (
	...
	fsLimitFlag    = kingpin.Flag("filesystems", "Regexp limiting the filesystems for which metrics are exported").Default(".*").String()
...
func main() {
	...
	fbCollector := collector.NewFlashbladeCollector(fbClient, *fsMetricFlag, *fsLimitFlag)
